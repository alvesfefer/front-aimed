...
export const api = {
  auth: {
    login: (email: string, password: string, role: string) =>
      fetch(`${API_URL}/${role}`, { // role = 'paciente' ou 'medico'
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password })
      }).then(handleResponse),

    register: (userData: Partial<User> & { password: string, role: string }) =>
      fetch(`${API_URL}/${userData.role}`, { // role = 'paciente' ou 'medico'
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    updateProfile: (id: string, data: Partial<User>) =>
      fetch(`${API_URL}/users/${id}`, { method: 'PUT', headers: getHeaders(), body: JSON.stringify(data) }).then(handleResponse)
  },
